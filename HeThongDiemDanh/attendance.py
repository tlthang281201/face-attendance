...
class Attendance:
    ...
    def __init__(self,root):
        ...
        img3 = img3.resize((1300, 690), PIL.Image.ANTIALIAS)
        self.photoimg3 = ImageTk.PhotoImage(img3)

        bg_img = Label(self.root, image=self.photoimg3)
        bg_img.place(x=0, y=0, width=1300, height=690)
        ...
        main_frame.place(x=23, y=90, width=1250, height=580)

        # left_label
        Left_frame = LabelFrame(main_frame, bd=2, bg="white", relief=RIDGE,
                                font=("times new roman", 12, "bold"))
        Left_frame.place(x=20, y=5, width=450, height=570)
        ...
        left_inside_frame.place(x=0, y=60, width=430, height=500)
        ...
        update_btn.place(x=100, y=450,width=250,height=43)


        btn_frame = Frame(left_inside_frame, bg="white")
        btn_frame.place(x=20, y=330, width=440, height=105)
        ...
        update_btn.grid(row=9, column=1,padx=10)
        ...
        # delete_btn.grid(row=9, column=1,pady=10)

        # reset_btn = Button(btn_frame, text="Làm mới",command=self.reset_data, font=("times new roman", 13, "bold"),
        #                    bg="#fbd568", fg="#996319", width=17)
        # reset_btn.grid(row=9, column=2,pady=10)

        label_Update_att = Button(btn_frame, text="Xem ảnh", command=self.openImage,
                                  font=("times new roman", 13, "bold"),
                                  bg="#38a6f0", fg="white", width=17)
        label_Update_att.grid(row=9, column=2,pady=10)


        #right_ label
        Right_frame = LabelFrame(main_frame, bd=2, bg="white",
                                 font=("times new roman", 12, "bold"))
        Right_frame.place(x=480, y=5, width=760, height=570)
        ...
        search_combo = ttk.Combobox(Right_frame, font=("times new roman", 11, "bold"),textvariable=self.var_com_search, state="readonly",
                                    width=13)
        ...
        search_btn = Button(Right_frame,command=self.search_data, text="Tìm kiếm", font=("times new roman", 9, "bold"), bg="#38a6f0", fg="white",
                            width=12)
        search_btn.grid(row=0, column=3, padx=5)

        Today_btn = Button(Right_frame, text="Hôm nay",command=self.today_data, font=("times new roman", 9, "bold"), bg="#38a6f0",
                             fg="white",
                             width=12)
        Today_btn.grid(row=0, column=4, padx=15)


        showAll_btn = Button(Right_frame, text="Xem tất cả", command=self.fetch_data,font=("times new roman", 9, "bold"), bg="#38a6f0",
                             fg="white",
                             width=12)
        ...
        table_frame.place(x=5, y=55, width=740, height=510)
        ...
        if self.var_com_search.get()=="" or self.var_search.get()=="":
            ...
            try:
                ...
                if(self.var_com_search.get()=="ID Điểm Danh"):
                    searchCol = "IdAuttendance"
                elif(self.var_com_search.get()=="Ngày"):
                    searchCol = "Date"
                else:
                    if(self.var_com_search.get()=="ID Sinh Viên"):
                        searchCol = "Student_id"
                    elif(self.var_com_search.get()=="ID Buổi học"):
                        searchCol = "Lesson_id"
                mydata.clear()
                my_cursor.execute("select * from attendance where "+str(searchCol)+" Like '%"+str(self.var_search.get())+"%'")
                data=my_cursor.fetchall()
                if(len(data)!=0):
