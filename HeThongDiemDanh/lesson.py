...
class Lesson:
    def __init__(self,root):
        ...
        Left_frame.place(x=0, y=3, width=400, height=570)

        label_Update_att = Label(Left_frame, bg="#F0FFF0", fg="#483D8B", text="Thông tin lịch học",
                                 font=("times new roman", 18, "bold"))
        label_Update_att.place(x=0, y=1, width=395, height=45)

        left_inside_frame = Frame(Left_frame, bd=1, bg="white")
        left_inside_frame.place(x=0, y=45, width=390, height=520)
        ...
        btn_frame.place(x=10, y=360, width=350, height=110)
        ...
        Right_frame = LabelFrame(main_frame, bd=2, bg="white",
                                 font=("times new roman", 12, "bold"))
        Right_frame.place(x=410, y=3, width=835, height=570)
        ...
        search_entry = ttk.Entry(Right_frame, textvariable=self.var_search, width=25,
        ...
        table_frame.place(x=5, y=55, width=820, height=500)
    ...
    def add_data(self):
        ...
        if self.var_id.get()=="" or self.var_timestart.get()=="" or self.var_timeend.get()=="" \
            ...
            try:
                ...
                self.reset_data()
                conn.close()
                messagebox.showinfo("Thành công","Thêm thông tin lịch học thành công",parent=self.root)
            ...
            try:
                ...
                if(self.var_com_search.get()=="ID GV"):
                    ...
                    if(self.var_com_search.get()=="Ngày học"):
                        search = "Date"
                my_cursor.execute("select * from lesson where "+str(search)+" Like '%"+str(self.var_search.get())+"%'")
                data=my_cursor.fetchall()
                if(len(data)!=0):
                    self.AttendanceReportTable.delete(*self.AttendanceReportTable.get_children())
                    for i in data:
