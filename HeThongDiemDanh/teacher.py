...
class Teacher:
    def __init__(self,root):
        ...
        Left_frame.place(x=20, y=5, width=420, height=570)

        label_Update_att = Label(Left_frame, bg="#F0FFF0", fg="#483D8B", text="Thông tin giáo viên",
                                 font=("times new roman", 18, "bold"))
        label_Update_att.place(x=0, y=1, width=417, height=45)

        left_inside_frame = Frame(Left_frame, bd=1, bg="white")
        left_inside_frame.place(x=0, y=60, width=400, height=500)
        ...
        btn_frame.place(x=20, y=330, width=440, height=115)
        ...
        Right_frame = LabelFrame(main_frame, bd=2, bg="white",
                                 font=("times new roman", 12, "bold"))
        Right_frame.place(x=450, y=5, width=790, height=570)
        ...
        table_frame.place(x=5, y=55, width=770, height=500)
            ...
            try:
                ...
                if(self.var_com_search.get()=="ID GV"):
                    ...
                    if(self.var_com_search.get()=="SĐT"):
                        search ="Phone"
                my_cursor.execute("select * from teacher where "+str(search)+" Like '%"+str(self.var_search.get())+"%'")
                data=my_cursor.fetchall()
                if(len(data)!=0):
                    self.AttendanceReportTable.delete(*self.AttendanceReportTable.get_children())
                    for i in data:
