...
class Report:
    def __init__(self,root):
        ...
        self.heading = Label(self.root, text=self.txt, font=("times new roman", 22, "bold"), bg="white", fg="black",
                             bd=0, relief=FLAT)
        self.heading.place(x=400, y=18, width=650,height=35)

        main_frame = Frame(bg_img, bd=2, bg="white")
        main_frame.place(x=20, y=60, width=1300, height=580)

        # ===================Top_label=====================
        Top_frame=LabelFrame(main_frame, bd=0, bg="white",
                                font=("times new roman", 12, "bold"))
        Top_frame.place(x=5,y=0,width=1300,height=120)
        #===================select_for_txt=================
        ...
        student_frame.place(x=5,y=0,width=320,height=120)
        ...
        att_frame.place(x=340, y=0, width=320, height=120)
        ...
        late_frame.place(x=670, y=0, width=320, height=120)
        ...
        late_frame.place(x=1000, y=0, width=320, height=120)
        ...
        Left_frame = LabelFrame(main_frame,bd=2, bg="white",
                               font=("times new roman", 12, "bold"))
        Left_frame.place(x=10, y=125, width=660, height=450)
        #topleft
        late_group=LabelFrame(Left_frame,bd=0,bg="white",text="Sinh viên đi muộn",font=("times new roman", 11, "bold"),fg="black",relief=RIDGE)
        late_group.place(x=2,y=0,width=640,height=440)
        ...
        tabletc_frame.place(x=10, y=38, width=622, height=350)
        ...
        Right_frame = LabelFrame(main_frame, bd=2, bg="white",
                                font=("times new roman", 12, "bold"))
        Right_frame.place(x=680, y=125, width=615, height=450)

        noatt_lbl = Label(Right_frame, bd=0, bg="white", text="Sinh viên không điểm danh", font=("times new roman", 14, "bold"),
                         fg="red2", )
        noatt_lbl.place(x=0, y=0, width=570, height=30)

        notinGroup=LabelFrame(Right_frame,bd=0,bg="white")
        notinGroup.place(x=0,y=35,width=590,height=400)
        ...
        tablenotin_frame.place(x=10, y=38, width=580, height=350)
        ...
        if self.var_com_searchlate.get()=="" or self.var_searchlate.get()=="":
            ...
            try:
                ...
                my_cursor.execute("select DISTINCT Student_id,`Name`,attendance.Date,Subject_name,lesson.Lesson_id,AttendanceStatus from attendance,`subject`,lesson where"
                                  ...
                                  +str(search)+" Like '%"+str(self.var_searchlate.get())+"%'")
                data=my_cursor.fetchall()
                if(len(data)!=0):
                    self.LateTable.delete(*self.LateTable.get_children())
                    for i in data:
        ...
        if self.var_com_searchnoatt.get()=="" or self.var_searchnoatt.get()=="":
            ...
            try:
                ...
                my_cursor.execute("select DISTINCT Student_id,`Name`,attendance.Date,Subject_name,lesson.Lesson_id,AttendanceStatus from attendance,`subject`,lesson where"
                                  ...
                                  +str(search)+" Like '%"+str(self.var_searchnoatt.get())+"%'")
                data=my_cursor.fetchall()
                if(len(data)!=0):
                    self.NoAttTable.delete(*self.NoAttTable.get_children())
                    for i in data:
        ...
        if self.var_com_searchNotin.get()=="" or self.var_searchNotin.get()=="":
            ...
            try:
                ...
                my_cursor.execute("select student.Student_id,Name,Date,Subject_name,Lesson_id from student,lesson,student_has_subject,`subject` where "
                                  ...
                                  +str(search)+" Like '%"+str(self.var_searchNotin.get())+"%'")
                data=my_cursor.fetchall()
                if(len(data)!=0):
                    self.NotInTable.delete(*self.NotInTable.get_children())
                    for i in data:
